...
import { CheckCircle, XCircle, TrendingUp, Flame, BookOpen, Trophy } from 'lucide-react'
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4 mb-8">
        <Card>
          <p className="text-xs font-medium text-stone-400 uppercase tracking-wide mb-1">Questions</p>
          <p className="text-3xl font-bold text-stone-900">{totalAnswered}</p>
        </Card>
        <Card>
          <p className="text-xs font-medium text-stone-400 uppercase tracking-wide mb-1">Accuracy</p>
          <p className={cn('text-3xl font-bold', accuracy >= 80 ? 'text-emerald-600' : accuracy >= 60 ? 'text-amber-600' : 'text-rose-600')}>
            {accuracy}%
          </p>
        </Card>
        <Card>
          <p className="text-xs font-medium text-stone-400 uppercase tracking-wide mb-1">Quizzes</p>
          <p className="text-3xl font-bold text-stone-900">{quizzesCompleted}</p>
        </Card>
        <Card>
          <p className="text-xs font-medium text-stone-400 uppercase tracking-wide mb-1 flex items-center gap-1">
            <Flame className="h-3.5 w-3.5 text-amber-500" /> Streak
          </p>
          <p className="text-3xl font-bold text-stone-900">{streak} days</p>
        </Card>
      </div>
            ...
            {progress.map((p) => (
              <Card key={p.week_number}>
                <div className="flex items-center justify-between mb-2">
                  ...
                  {p.quiz_score != null && (
                    <span className="text-sm font-semibold text-sky-700 bg-sky-50 px-2 py-1 rounded-lg">
              ...
              {recent.map((a) => {
                ...
                return (
                  <div key={a.id} className="flex items-center gap-3 py-2.5">
                    ...
                    <p className="text-sm text-stone-700 flex-1 truncate">
                      {q?.question_text ?? 'Question'}
                    </p>
                    <span className="text-xs text-stone-400 flex-shrink-0">
                      {(q?.topic as Topic | undefined)?.title ?? ''}
                    </span>
